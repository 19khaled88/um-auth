...
import {
  ...
  academicSemesterSearchableFiels,
...
const getAllAcademicSemester = async (
  ...
  let result = await AcademicSemester.find({
  ...
  let total = await AcademicSemester.countDocuments({
    $and: andCondition.length > 0 ? andCondition : [{}],
  });
  ...
  if (result.length === 0 && searchTerm) {
    ...
    if (!isNaN(searchTermAsNumber)) {
      ...
      total = await AcademicSemester.countDocuments({
        $and: andCondition.length > 0 ? andCondition : [{}],
      });
    }
  }
...
const updateAcademicSemester = async (
  ...
  if (
    ...
    throw new ApiError(
      400,
      "Semester must match with relevent code requirement"
    );
  } else {
...
const createSemesterFromEvents = async (
...
const updateSemesterFromEvents = async (e:IAcademicSemesterFromEvent): Promise<void> => {
    await AcademicSemester.findOneAndUpdate({ syncId: e.id },{
      $set:{
          title: e.title,
          year: e.year,
          code: e.code,
...
const deleteSemesterFromEvents = async (e:IAcademicSemesterFromEvent): Promise<void> => {
