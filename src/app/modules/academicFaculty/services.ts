...
const getAllAcademicFaculties = async (
  ...
  let result = await AcademicFaculty.find({
  ...
  let total = await AcademicFaculty.countDocuments({
    $and: andCondition.length > 0 ? andCondition : [{}],
  });
...
const updateFacultyFromEvents = async (
  e: IAcademicFacultyFromEvents
): Promise<void> => {
...
const deleteFacultyFromEvents = async (
  e: IAcademicFacultyFromEvents
): Promise<void> => {
